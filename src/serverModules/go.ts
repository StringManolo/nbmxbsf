...
const go = (text: string): string => {
  const fd = open("./internalGoEvaling.go", "w");
  fd.puts(text.substring(4, text.length));
  fd.close();
  const output = run(`go run ./internalGoEvaling.go 2>&1`);
  run("rm ./internalGoEvaling.go 2>&1 > /dev/null");
  if (!output) {
